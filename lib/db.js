...
function getConfig() {
  const region = env("AWS_REGION", "SPACEKAYSONKELLY_AWS_REGION");
  const roleArn = env("AWS_ROLE_ARN", "SPACEKAYSONKELLY_AWS_ROLE_ARN");
  const tableName = env(
    "DYNAMODB_TABLE_NAME",
...
function getDocClient() {
  ...
  if (!region || !roleArn) {
    throw new Error(
      "Missing AWS_REGION or AWS_ROLE_ARN (or SPACEKAYSONKELLY_* equivalents)"
    );
  }
